...
Compatibility note: the qualified internal Python package remains `qwen_dual_server`, and protected runtime configuration still contains the historical internal service/lock identifier `qwen3-dual-4b-cpu-rest-server`. Those strings are retained for qualification compatibility and are not the public distribution identity.
...
The frozen Stage-II qualified source carries the internal label `v0.2.3c`; it was never published. Publication-hygiene work for `1.0.0` is limited to package identity, licensing, documentation, governance files, manifest/build hygiene, and other public-facing metadata unless a future change explicitly reopens qualification.

The protected Stage-II semantic files are tracked in `PRODUCTION_DEMO_PROVENANCE.md` and `CONTRIBUTING.md`.

See also `PRE_PUBLISH_NOTES.md` and `VERIFICATION_SUMMARY.txt` for the distinction between qualification provenance and first-public-release identity.
